...
import { dynamicFindMany, dynamicFindUnique, dynamicCreate, dynamicBulkCreate, dynamicUpdate, dynamicDelete, dynamicFindManyWithFilters, } from "../utils/dynamicDbOperations.js";
...
export class StockService {
     ...
     */
    async createBulkDirect(dataArray, options = {}) {
    ...
    async createBulkOptimized(dataArray, options = {}) {
        ...
        for (let i = 0; i < dataArray.length; i += batchSize) {
            ...
            try {
                ...
                const batchResults = await this.processBatchWithConcurrency(batch, i, // starting index for error reporting
                { maxConcurrency });
     ...
     * Uses single SQL query with VALUES clause for maximum performance
     */
    async dynamicBulkCreate(modelName, dataArray) {
        logger.debug({
            modelName,
     ...
     */
    async processBatchWithConcurrency(batch, startIndex, options) {
     ...
     */
    async createBulkAsync(dataArray, options = {}) {
     ...
     */
    async updateProductAndPlatformStockForBulkInsert(insertedStocks) {
        ...
        for (const [groupKey, stocks] of platformStockGroups) {
            try {
                ...
                if (products && products.length > 0) {
                    ...
                    await platformStockService.updatePlatformStockQuantities(product.id, platform, {
     ...
     */
    async processAsyncBulkJob(jobId, dataArray, options) {
