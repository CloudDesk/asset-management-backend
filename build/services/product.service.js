...
export class ProductService {
        ...
        try {
            ...
            if (comboProductIds.length > 0) {
                try {
                    ...
                    const componentProductIds = [...new Set(allComponents.map((comp) => comp.componentproductid))];
                    ...
                    const componentPlatformStocks = componentProductIds.length > 0
                        ? await prisma.platformStock.findMany({
                            where: {
                                productid: { in: componentProductIds.map((id) => BigInt(id)) },
                                platform: 'nivapp'
                            },
                            select: {
                                productid: true,
                    ...
                    componentPlatformStocks.forEach((ps) => {
        ...
        try {
            ...
            if (product.iscombo === true) {
                try {
                    ...
                    product.components = components.map((comp) => ({
                        componentproductid: Number(comp.componentproductid),
                        requiredqty: comp.requiredqty,
                        isactive: comp.isactive,
                        product: {
            ...
            logger.debug({
                ...
                componentCount: product.iscombo ? (product.components?.length || 0) : 0
        ...
        try {
            ...
            if (product.iscombo === true) {
                try {
                    ...
                    const componentProductIds = components.map((comp) => comp.componentproductid);
    ...
    async create(data) {
        try {
            ...
            if (isCombo) {
                ...
                if (existingCombo) {
                    throw new Error(`A combo product with the same components already exists. ` +
                ...
                for (const component of components) {
                    ...
                    if (componentProduct.iscombo === true) {
     ...
     */
    async findExistingComboByComponents(components) {
        try {
            ...
            const normalizedComponents = components
                ...
                if (a.componentproductid < b.componentproductid)
                    return -1;
                if (a.componentproductid > b.componentproductid)
                    return 1;
            ...
            const comboProducts = await prisma.$queryRaw `
        SELECT id, name
        FROM product
        WHERE iscombo = true
      `;
            ...
            for (const comboProduct of comboProducts) {
                ...
                const bundleMaps = await prisma.$queryRaw `
        ...
        `;
            ...
            logger.error({ error: error.message, components }, 'Error checking for existing combo product');
     ...
     */
    async validateComboComponents(components) {
        try {
            if (!components || !Array.isArray(components) || components.length === 0) {
                return {
                    isValid: false,
            ...
            logger.error({ error: error.message, components }, 'Error validating combo components');
            return {
                isValid: false,
