...
export class PromotionalAssetsService {
    async auditLog(assetId, action, changedBy, changes) {
        try {
    ...
    async findMany(filters, page, limit) {
        try {
            ...
            else if (filters.priority_min !== undefined ||
            ...
            if (filters.schedule_start) {
                logger.debug("Processing schedule_start filter:", filters.schedule_start);
                try {
                    const startDate = new Date(filters.schedule_start);
                    if (isNaN(startDate.getTime())) {
                        logger.error("Invalid schedule_start date:", filters.schedule_start);
    ...
    async update(id, data, userId) {
        try {
            ...
            if (data.version && existingAsset.version !== data.version) {
                throw new ValidationError("Asset has been modified by another user. Please refresh and try again.", "Concurrency conflict detected");
            }
            ...
            if (updateData.content) {
                ...
                if (updateData.content.images && Array.isArray(updateData.content.images)) {
                    ...
                    const uniqueNewImages = newImages.filter((img) => !existingImages.includes(img));
    ...
    async upsert(data, userId) {
        try {
            ...
            if (data.id) {
                ...
                const newAsset = await this.create(createData, userId);
                return {
                    asset: newAsset,
                    operation: 'created'
                };
    ...
    async deleteImage(id, imageUrl, userId) {
        try {
            const existingAsset = await this.findById(id);
            ...
            logger.info({ assetId: id, imageUrl, userId }, "Image deleted from promotional asset successfully");
            return convertBigIntToNumber(updatedAsset);
        }
        catch (error) {
    ...
    async getAuditLogs(assetId, page = 1, limit = 20) {
        try {
            const { skip, take } = getPrismaSkipTake(page, limit);
...
//# sourceMappingURL=promotional-assets.service.js.map