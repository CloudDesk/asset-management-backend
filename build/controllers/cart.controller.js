...
import { createCartSchema, updateCartSchema, upsertCartSchema, cartParamsSchema } from '../schemas/cart.schema.js';
...
export class CartController {
    ...
    getCarts = asyncHandler(async (request, reply) => {
    ...
    getCart = asyncHandler(async (request, reply) => {
    ...
    createCart = asyncHandler(async (request, reply) => {
    ...
    deleteCart = asyncHandler(async (request, reply) => {
        const { id } = cartParamsSchema.parse(request.params);
        await this.cartService.delete(id);
        const response = createSuccessResponse('Cart item deleted successfully', null);
        return reply.code(200).send(response);
    });
    upsertCart = asyncHandler(async (request, reply) => {
    ...
    getCartsByUserId = asyncHandler(async (request, reply) => {
    ...
    getWishlistByUserId = asyncHandler(async (request, reply) => {
        const { userId } = request.params;
        if (!userId || !/^\d+$/.test(userId)) {
            return reply.code(400).send({
                success: false,
    ...
    clearCartByUserId = asyncHandler(async (request, reply) => {
        const { userId } = request.params;
        if (!userId || !/^\d+$/.test(userId)) {
            return reply.code(400).send({
                success: false,
