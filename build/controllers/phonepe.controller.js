    ...
    initiatePayment = asyncHandler(async (request, reply) => {
        try {
            ...
            if (result.success) {
                ...
                const response = createSuccessResponse(requestBody.mode === "phonepe"
                    ...
                    status: requestBody.mode === "phonepe"
                        ...
                        : "COD_ORDER_CREATED",
                    mode: requestBody.mode,
                    message: requestBody.mode === "phonepe"
                ...
                try {
                    ...
                    try {
                        ...
                        if (transactions.data && transactions.data.length > 0) {
                            ...
                            console.log("originalOrderItems:", JSON.stringify(originalOrderItems, null, 2));
                            console.log(transaction, "transaction ==>> Test ==> ");
                            console.log(originalOrderItems, "originalOrderItems from transaction ==>> Test ==> ");
                            if (originalOrderItems.length > 0) {
                                console.log("=== CALLING updateProductQuantitiesAfterOrder ===");
                                ...
                                logger.info({
                                    ...
                                    originalOrderItems: originalOrderItems.map((item) => ({
                                        productid: item.productid,
                                        quantity: item.quantity,
                                        productname: item.productname,
                                    })),
                                ...
                                console.log("quantityUpdateResult:", JSON.stringify(quantityUpdateResult, null, 2));
                                logger.info({
                                    merchantTransactionId,
                                    orderId: orderData.id,
                                    mode: "phonepe",
                                ...
                                logger.warn({
                                    ...
                                    originalPayload: transaction.transactiondata?.originalPayload,
                                }, "No original order items found for quantity update");
                            }
                        }
                        else {
                            console.log("=== NO TRANSACTIONS FOUND ===");
                            console.log("No transactions found for merchantTransactionId:", merchantTransactionId);
                        }
                    }
                    catch (quantityUpdateError) {
                        console.log("=== QUANTITY UPDATE ERROR ===");
                        console.log("quantityUpdateError:", JSON.stringify(quantityUpdateError, null, 2));
                        logger.error({
                            error: quantityUpdateError.message,
                            merchantTransactionId,
                            orderId: orderData?.id,
        ...
        try {
            ...
            for (const orderItem of originalOrderItems) {
                try {
                    ...
                    console.log(verificationProduct, "verificationProduct ==>> Test ==> ");
