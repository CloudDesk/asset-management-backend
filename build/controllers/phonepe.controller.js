...
export class PhonePeController {
                        ...
                        try {
                            ...
                            for (const component of components) {
                                ...
                                if (!componentPlatformStock) {
                                    componentValidationErrors.push(`Component ${componentProductId} (${component.componentproduct?.name || "Unknown"}) has no platformstock record`);
                                    continue;
                                }
                                ...
                                if (actualAvailable < totalNeeded) {
                                    componentValidationErrors.push(`Component "${component.componentproduct?.name || componentProductId}" needs ${totalNeeded} units (${requiredQty} per combo × ${requestedQuantity} combos), but only ${actualAvailable} available`);
                                }
                            }
                ...
                await prisma.$transaction(async (tx) => {
                    for (const orderItem of requestBody.order) {
                        try {
                            ...
                            if (!product) {
                                throw new Error(`Product ${productId} not found (should have been caught in validation)`);
                            }
                            // ========================================
                            ...
                            if (product.iscombo) {
                                ...
                                const comboLockResults = await this.lockComboComponents(tx, productId, requestedQuantity, orderItem);
                  ...
                  `;
                            ...
                            if (itemError.code === "INSUFFICIENT_COMBO_STOCK") {
                                ...
                                logger.error(errorContext, "Failed to lock stock for product");
                                lockErrors.push({
                                    productId: orderItem.productid,
                                    productName: orderItem.productname,
                                    error: itemError.message,
        ...
        try {
            ...
            if (createdOrderlines.length > 0) {
                try {
                    ...
                    const totalShippingCost = roundToTwo(parseFloat(transaction.transactiondata?.originalPayload?.shippingCost?.toString() ||
                        "0"));
                    // Get order-level promotion discount total for proportional distribution
                    const orderPromotionDiscountTotal = roundToTwo(parseFloat(order.promotion_discount_total?.toString() || "0"));
                    const orderProductDiscountTotal = roundToTwo(Number(productDiscountTotal) || 0);
                    const orderTotalDiscountAmount = roundToTwo(parseFloat(order.discountamount?.toString() || "0"));
                    const orderProductAmountTotal = roundToTwo(parseFloat(order.productamount?.toString() || "0"));
                    const orderOrderAmountTotal = roundToTwo(parseFloat(order.orderamount?.toString() || "0"));
                    const orderShippingTotal = roundToTwo(parseFloat(order.shipping_cost?.toString() || "0"));
                    ...
                    for (const orderline of createdOrderlines) {
                        const lineOrderAmount = roundToTwo(parseFloat(orderline.orderamount?.toString() || "0"));
                        const lineProductAmount = roundToTwo(parseFloat(orderline.productamount?.toString() || "0"));
                        const lineQuantity = parseFloat(orderline.quantity?.toString() || "1");
                        const lineShipping = roundToTwo(parseFloat(orderline.shipping_cost?.toString() || "0"));
                        totalOrderAmount = roundToTwo(totalOrderAmount + lineOrderAmount);
                        totalProductAmountFromLines = roundToTwo(totalProductAmountFromLines + lineProductAmount);
                    ...
                    const promotionDistributionBasis = orderPromotionDiscountTotal > 0
                        ? totalOrderAmount > 0
                            ? totalOrderAmount
                    ...
                    if (orderPromotionDiscountTotal > 0 && promotionDistributionBasis > 0) {
                        for (let i = 0; i < orderlineAmounts.length; i++) {
                            ...
                            if (i === orderlineAmounts.length - 1) {
                                linePromoDiscount = roundToTwo(orderPromotionDiscountTotal - allocatedPromoDiscount);
                            }
                    ...
                    for (let i = 0; i < createdOrderlines.length; i++) {
                        ...
                        const originalPricePerItem = roundToTwo(originalPriceMap.get(productId) ||
                            (lineQuantity > 0
                                ? (orderlineAmounts[i]?.productamount || 0) / lineQuantity
                                : 0));
                        const productDiscountPerItem = roundToTwo(productDiscountMap.get(productId) || 0);
                        let productDiscountAmount = roundToTwo(productDiscountPerItem * lineQuantity);
                        let promotionDiscountAmount = roundToTwo(promoDiscounts[i]?.discount || 0);
                        const originalPriceTotal = roundToTwo(originalPricePerItem * lineQuantity);
                        let productAmountOnly = roundToTwo(originalPriceTotal - productDiscountAmount);
                        let totalDiscountForLine = roundToTwo(productDiscountAmount + promotionDiscountAmount);
                        let finalPriceTotal = roundToTwo(productAmountOnly - promotionDiscountAmount);
                        let lineShippingCost = roundToTwo(orderProductAmountTotal > 0
                            ? totalShippingCost * (productAmountOnly / orderProductAmountTotal)
                            : 0);
                        if (isLastLine) {
                            promotionDiscountAmount = roundToTwo(orderPromotionDiscountTotal - accumulatedPromotion);
                        ...
                        if (finalPriceTotal < 0) {
                            ...
                            promotionDiscountAmount = roundToTwo(Math.max(0, productAmountOnly));
                            totalDiscountForLine = roundToTwo(productDiscountAmount + promotionDiscountAmount);
                            finalPriceTotal = 0;
                        }
                        const updateData = {
                            evaluation_id: primaryEvaluationId,
                        ...
                        accumulatedPromotion = roundToTwo(accumulatedPromotion + promotionDiscountAmount);
                        accumulatedProductDiscount = roundToTwo(accumulatedProductDiscount + productDiscountAmount);
                        accumulatedDiscountAmount = roundToTwo(accumulatedDiscountAmount + totalDiscountForLine);
                        accumulatedOrderAmount = roundToTwo(accumulatedOrderAmount + finalPriceTotal);
                        accumulatedProductAmount = roundToTwo(accumulatedProductAmount + productAmountOnly);
                        accumulatedShippingAmount = roundToTwo(accumulatedShippingAmount + lineShippingCost);
                    }
                }
                catch (updateError) {
                    logger.warn({
            ...
            try {
                ...
                const cartClearResult = await cartService.clearCartByUserId(transaction.userid.toString());
                logger.info({
                    transactionId,
                    orderId: order.id,
                    userId: transaction.userid,
            ...
            for (const orderItem of originalOrderItems) {
                try {
                    ...
                    if (productCheck.iscombo) {
                        ...
                        try {
                            const conversionResult = await this.convertComboComponentLocksToOrders(productId, requestedQuantity, orderData, orderItem, PLATFORM_NAME);
                            updateResults.push({
                                productId: productId,
                                productName: productCheck.name,
                                productType: "combo",
                ...
                await prisma.$transaction(async (tx) => {
                    for (const item of orderItems) {
                        try {
                            ...
                            if (product.iscombo) {
                                const comboReleaseResults = await this.releaseComboComponentLocks(tx, item.productid, item.quantity, PLATFORM_NAME, merchantTransactionId);
                                releaseResults.push({
                                    productId: item.productid,
                                    productName: product.name,
                                    productType: "combo",
     ...
     */
    async convertComboComponentLocksToOrders(comboProductId, comboQuantity, orderData, orderItem, platformName) {
        logger.info({
            comboProductId,
            comboQuantity,
            orderId: orderData.id,
        ...
        if (!components || components.length === 0) {
            throw new Error(`No active components found for combo product ${comboProductId}`);
        }
        logger.info({
            comboProductId,
            componentCount: components.length,
        ...
        for (const component of components) {
            ...
            if (!platformStock) {
                throw new Error(`PlatformStock not found for component ${componentProductId} (${component.componentproduct?.name})`);
            }
            const currentLockQty = platformStock.lockqty || 0;
            const currentOrderedQty = platformStock.orderedqty || 0;
            const currentAvailableQty = platformStock.availableqty || 0;
            ...
            if (quantityToConvert < totalNeeded) {
                logger.warn({
                    ...
                    currentLockQty,
                    quantityToConvert,
                    warning: "Locked quantity less than expected - using available lock only",
     ...
     */
    async lockComboComponents(tx, comboProductId, comboQuantity, orderItem) {
        const PLATFORM_NAME = "nivapp";
        logger.info({
            comboProductId,
            comboQuantity,
      ...
      `;
            if (!platformStockResult || platformStockResult.length === 0) {
                validationErrors.push(`PlatformStock not found for component ${componentProductId} (${component.componentproduct?.name || "Unknown"})`);
                continue;
            }
            const platformStock = platformStockResult[0];
            const currentAvailableQty = Number(platformStock.availableqty) || 0;
            ...
            // Note: availableqty is the actual available stock, so we check it directly
            if (actualAvailable < totalNeeded) {
                validationErrors.push(`Insufficient stock for component ${componentProductId} (${component.componentproduct?.name || "Unknown"}). ` +
        ...
        if (validationErrors.length > 0) {
            ...
            throw new ValidationError(`Insufficient stock for combo product ${comboProductId}. ` +
                `Components with insufficient stock: ${validationErrors.join("; ")}`, `INSUFFICIENT_COMBO_STOCK - Combo Product ID: ${comboProductId}, Quantity: ${comboQuantity}`, validationErrors);
        }
    ...
    async releaseComboComponentLocks(tx, comboProductId, comboQuantity, platformName, merchantTransactionId) {
        const components = await tx.productBundleMap.findMany({
            where: {
                bundleproductid: BigInt(comboProductId),
                isactive: true,
      ...
      `;
            if (!platformStockResult || platformStockResult.length === 0) {
                throw new Error(`PlatformStock not found for combo component ${componentProductId}`);
            }
            const platformStock = platformStockResult[0];
            const currentAvailableQty = Number(platformStock.availableqty) || 0;
            const currentLockQty = Number(platformStock.lockqty) || 0;
