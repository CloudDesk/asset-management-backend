...
export class PhonePeController {
    ...
    handlePaymentCallback = asyncHandler(async (request, reply) => {
        try {
            ...
            // Perform redirect using HTML with JavaScript for better mobile webview compatibility
            const redirectHtml = `
<!DOCTYPE html>
<html>
<head>
...
<body>
  <div class="container">
    ...
    <h2>${result.success ? 'Payment Successful!' : 'Payment Processing...'}</h2>
    <p>Redirecting you back to the app...</p>
  </div>
  <script>
    // Immediate redirect for mobile webview compatibility
    window.location.href = "${result.redirectUrl}?merchantTransactionId=${merchantTransactionId}&status=${result.success ? 'success' : 'failed'}";
    
    // Fallback in case immediate redirect doesn't work
    setTimeout(function() {
      window.location.replace("${result.redirectUrl}?merchantTransactionId=${merchantTransactionId}&status=${result.success ? 'success' : 'failed'}");
    }, 100);
  </script>
</body>
</html>`;
            return reply.type('text/html').send(redirectHtml);
        }
        catch (error) {
            logger.error({
                error: error.message,
            ...
            // Send HTML redirect instead of server redirect for mobile webview compatibility
...
<body>
  ...
  <script>
    window.location.href = "${failureUrl}?merchantTransactionId=${request.params?.merchantTransactionId || ''}&status=error";
    setTimeout(function() {
      window.location.replace("${failureUrl}?merchantTransactionId=${request.params?.merchantTransactionId || ''}&status=error");
    }, 100);
  </script>
</body>
</html>`;
            return reply.type('text/html').send(errorRedirectHtml);
        }
    });
    /**
            ...
            if (evaluationData && createdOrderlines.length > 0) {
                try {
                    ...
                    const totalShippingCost = parseFloat(transaction.transactiondata?.originalPayload?.shippingCost?.toString() ||
                        "0");
                    const shippingCostPerItem = createdOrderlines.length > 0
