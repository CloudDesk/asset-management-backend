...
export class PicklistController {
     ...
     */
    getPicklistsV2 = asyncHandler(async (request, reply) => {
        const allFilters = request.query || {};
        ...
        const sortorder = (allFilters.sortorder?.toUpperCase() === 'DESC' ? 'DESC' : 'ASC');
        const fieldnameOrder = (allFilters.fieldnameOrder?.toUpperCase() === 'DESC' ? 'DESC' : 'ASC');
        ...
        const result = await this.picklistService.findManyV2(filters, groupByFieldname, groupByParent, sortorder, fieldnameOrder, limit);
        ...
        const response = createSuccessResponse('Picklists retrieved successfully', result.flat || []);
        return reply.code(200).send({
            ...response,
            pagination: result.pagination,
            meta: result.meta
     ...
     */
    bulkUpdatePicklistsV2 = asyncHandler(async (request, reply) => {
     ...
     */
    getDependencyFieldnames = asyncHandler(async (request, reply) => {
        ...
        const response = createSuccessResponse(`Retrieved ${fieldnames.length} unique fieldname(s) for object '${object}'`, fieldnames);
        return reply.code(200).send(response);
    });
}
