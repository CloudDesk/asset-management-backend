...
let purchaseRequestSchemas = null;
let lastGenerated = 0;
const CACHE_TTL = 30 * 60 * 1000; // 30 minutes
/**
 * Gets the dynamically generated purchase request schemas
 */
export async function getPurchaseRequestSchemas() {
    try {
        // Check if we need to regenerate schemas
        if (!purchaseRequestSchemas || (Date.now() - lastGenerated) > CACHE_TTL) {
            logger.debug('Generating fresh purchase request schemas');
            purchaseRequestSchemas = await generateCRUDSchemas('purchaserequest');
            lastGenerated = Date.now();
            logger.info('Purchase request schemas generated successfully');
        }
        return purchaseRequestSchemas;
    }
    catch (error) {
        logger.error({ error }, 'Failed to generate purchase request schemas');
        throw error;
    }
}
/**
 ...
 */
export async function getPurchaseRequestQuerySchema() {
    try {
        const schemas = await getPurchaseRequestSchemas();
        const responseSchema = schemas.response;
        // Generate query parameters based on actual purchase request fields
        ...
        // Add filter parameters for each field in the purchase request schema
        ...
        logger.error({ error }, 'Failed to generate purchase request query schema');
        throw error;
    }
}
/**
 ...
 */
export function clearPurchaseRequestSchemaCache() {
    purchaseRequestSchemas = null;
    lastGenerated = 0;
    logger.info('Purchase request schema cache cleared');
}
//# sourceMappingURL=purchaserequest.swagger.js.map