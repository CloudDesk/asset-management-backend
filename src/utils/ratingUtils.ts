...
export async function updateProductAverageRating(
  ...
  try {
    logger.debug(
      { productId },
      "Starting product average rating update via utility"
    );
    ...
    logger.error(
      { error, productId },
      "Error in product average rating update utility"
    );
    throw error;
  }
}
 ...
 */
export async function safeUpdateProductAverageRating(
  productId: number,
  context: { operation: string; ratingId?: string | number }
): Promise<void> {
  try {
