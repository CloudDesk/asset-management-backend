...
const safeColumnsCache = new Map<
  string,
  { columns: string[]; columnList: string; lastChecked: number }
>();
 ...
 */
async function getSafeColumnsForTable(
  tableName: string
): Promise<{ columns: string[]; columnList: string }> {
  // Check cache first
...
async function discoverTableColumns(tableName: string): Promise<string[]> {
  try {
    ...
    logger.info(
      { tableName, columns },
      `Discovered ${columns.length} columns for table ${tableName}`
    );
    return columns;
  } catch (error) {
...
async function filterInputDataBySchema(
  ...
  writeFileSync(
    "debug_filter_start.txt",
    `Filtering data for ${modelName} (${tableName}): ${JSON.stringify(
      data,
      null,
      2
    )}\n`,
  ...
  writeFileSync(
    "debug_filter_columns.txt",
    `Available columns for ${tableName}: ${JSON.stringify(availableColumns)}\n`,
    { flag: "a" }
  );
  ...
  if (availableColumns.length === 0) {
    logger.warn(
      { modelName, tableName },
      "No columns available, returning empty data"
    );
    writeFileSync(
      "debug_filter_no_columns.txt",
  ...
  for (const [key, value] of Object.entries(data)) {
    if (availableColumns.includes(key)) {
      ...
      writeFileSync(
        "debug_filter_included.txt",
        `Included field: ${key} = ${JSON.stringify(value)}\n`,
        { flag: "a" }
      );
    } else {
      ignoredFields.push(key);
      writeFileSync(
        "debug_filter_ignored.txt",
        `Ignored field: ${key} (not in available columns)\n`,
        { flag: "a" }
      );
    }
  }
  ...
  writeFileSync(
    "debug_filter_result.txt",
    `Filtered data result: ${JSON.stringify(filteredData, null, 2)}\n`,
    { flag: "a" }
  );
  writeFileSync(
    "debug_filter_ignored_summary.txt",
    `Ignored fields: ${JSON.stringify(ignoredFields)}\n`,
    { flag: "a" }
  );
...
export async function fastFindMany(
  ...
  try {
    ...
    logger.debug(
      { query, tableName, fast: !useAllColumns },
      "Executing fast findMany query"
    );
...
async function buildDynamicWhereClause(
  ...
  function findMatchingColumn(filterKey: string): string | null {
    ...
    const camelKey = filterKey.replace(/_([a-z])/g, (_, letter) =>
      letter.toUpperCase()
    );
    ...
    const matchingColumn = availableColumns.find(
      (col) => col.toLowerCase() === filterKey.toLowerCase()
    );
    if (matchingColumn) {
      ...
      if (value !== undefined && value !== null && value !== "") {
        ...
        if (key.startsWith("min") && key.length > 3) {
          ...
          if (processedValue.includes("%") || processedValue.includes("*")) {
            ...
            conditions.push(
              `LOWER("${matchingColumn}") = LOWER($${paramIndex})`
            );
...
export async function dynamicFindManyWithFilters(
  ...
  try {
    ...
    // Build WHERE clause
    const { whereClause, values } = await buildDynamicWhereClause(
      tableName,
      filters
    );
...
export async function dynamicFindMany(
  ...
  try {
    ...
    if (availableColumns.length === 0) {
      logger.warn(
        { modelName, tableName },
        "No columns available for findMany"
      );
      return [];
    }
    ...
    if (!options.where || Object.keys(options.where).length === 0) {
      logger.debug(
        { tableName, skip, take },
        "Executing optimized raw SQL SELECT query"
      );
      ...
      const query = `SELECT ${columnList} FROM ${tableName} ORDER BY ${orderByClause} LIMIT ${take} OFFSET ${skip}`;
      logger.debug(
        { query, tableName, orderByClause },
        "Executing optimized raw SQL query"
      );
...
export async function dynamicCount(
  ...
  try {
    ...
    logger.error(
      { error: error.message, modelName, where },
      "Error in dynamic count operation"
    );
    return 0;
  }
}
...
export async function dynamicFindUnique(
  ...
  try {
    ...
    if (availableColumns.length === 0) {
      logger.warn(
        { modelName, tableName },
        "No columns available for findUnique"
      );
      return null;
    }
    ...
    try {
      ...
      if (modelName === "product") {
        ...
        if (result && !result.hasOwnProperty("isdealoftheday")) {
          logger.debug(
            { modelName },
            "Prisma result missing isdealoftheday field, falling back to raw SQL"
          );
    ...
    logger.error(
      { error: error.message, modelName, where },
      "Error in dynamic findUnique operation"
    );
    return null;
  }
}
  ...
  try {
    ...
    for (const [key, value] of Object.entries(filteredData)) {
      if (availableColumns.includes(key)) {
        ...
        if (
          (key === "paymentdata" || key === "items" || key === "content") &&
          value !== null &&
          value !== undefined
        ) {
    ...
    const { columnList: safeColumnsList } = await getSafeColumnsForTable(
      tableName
    );
...
export async function dynamicUpdate(
  ...
  try {
    ...
    if (Object.keys(filteredData).length === 0) {
      logger.warn(
        { modelName, where, originalData: data },
        "No valid fields for update operation"
      );
      return null;
    }
    ...
    try {
      ...
      const { writeFileSync } = await import("fs");
      writeFileSync(
        "debug_raw_sql_start.txt",
        `Starting raw SQL update for ${modelName} (${tableName})\n`,
        { flag: "a" }
      );
      writeFileSync(
        "debug_raw_sql_filtered_data.txt",
        `Filtered data: ${JSON.stringify(filteredData, null, 2)}\n`,
        { flag: "a" }
      );
      writeFileSync(
        "debug_raw_sql_available_columns.txt",
      ...
      for (const [key, value] of Object.entries(filteredData)) {
        if (availableColumns.includes(key)) {
          ...
          if (
            ...
            if (key === "paymentdata") {
              writeFileSync(
                "debug_raw_sql_paymentdata.txt",
                `Converted paymentdata: ${rawData[key]}\n`,
                { flag: "a" }
              );
            } else if (key === "items") {
              writeFileSync(
                "debug_raw_sql_items.txt",
                `Converted items: ${rawData[key]}\n`,
                { flag: "a" }
              );
            }
          } else {
          ...
          writeFileSync(
            "debug_raw_sql_included.txt",
            `Included in rawData: ${key} = ${JSON.stringify(value)}\n`,
            { flag: "a" }
          );
        } else {
          writeFileSync(
            "debug_raw_sql_excluded.txt",
            `Excluded from rawData: ${key} (not in available columns)\n`,
            { flag: "a" }
          );
        }
      }
      ...
      writeFileSync(
        "debug_raw_sql_rawdata.txt",
        `Final rawData: ${JSON.stringify(rawData, null, 2)}\n`,
        { flag: "a" }
      );
      ...
      if (Object.keys(rawData).length === 0) {
        logger.warn(
          { modelName, tableName },
          "No valid columns for raw SQL update"
        );
        writeFileSync(
          "debug_raw_sql_no_data.txt",
          `No valid columns for raw SQL update\n`,
          { flag: "a" }
        );
        return null;
      ...
      if (availableColumns.includes("modifieddate")) {
        rawData.modifieddate = now;
        writeFileSync(
          "debug_raw_sql_modifieddate.txt",
      ...
      writeFileSync(
        "debug_raw_sql_id.txt",
        `ID value: ${idValue} (type: ${typeof idValue})\n`,
        { flag: "a" }
      );
      ...
      writeFileSync(
        "debug_raw_sql_query.txt",
        `Update query: ${updateQuery}\n`,
        { flag: "a" }
      );
      writeFileSync(
        "debug_raw_sql_values.txt",
      ...
      try {
        ...
        writeFileSync(
          "debug_raw_sql_updated_record.txt",
        ...
        writeFileSync(
          "debug_raw_sql_no_record.txt",
          `No record returned from update\n`,
          { flag: "a" }
        );
        return null;
      } catch (sqlError: any) {
        writeFileSync(
          "debug_raw_sql_error.txt",
      ...
      const { writeFileSync } = await import("fs");
      writeFileSync(
        "debug_raw_sql_fallback_start.txt",
        `Starting raw SQL fallback update for ${modelName} (${tableName})\n`,
        { flag: "a" }
      );
      ...
      if (Object.keys(rawData).length === 0) {
        logger.warn(
          { modelName, tableName },
          "No valid columns for raw SQL fallback update"
        );
        writeFileSync(
          "debug_raw_sql_fallback_no_data.txt",
          `No valid columns for raw SQL fallback update\n`,
          { flag: "a" }
        );
        return null;
      ...
      writeFileSync(
        "debug_raw_sql_fallback_query.txt",
        `Fallback query: ${updateQuery}\n`,
        { flag: "a" }
      );
      writeFileSync(
        "debug_raw_sql_fallback_values.txt",
        `Fallback values: ${JSON.stringify(values, null, 2)}\n`,
        { flag: "a" }
      );
...
export async function dynamicDelete(
  ...
  try {
    ...
    if (!availableColumns.includes("id")) {
      logger.warn(
        { modelName, tableName, columns: availableColumns },
 ...
 */
export function getSchemaCacheStatus(): Array<{
  tableName: string;
  columns: string[];
  age: number;
}> {
...
export function formatSupplierForAPI(supplier: any): any {
  ...
  if (formatted.supplierphonenumber !== undefined) {
    formatted.supplierphonenumber = formatIntegerField(
      formatted.supplierphonenumber
    );
  }
...
export function formatProductForAPI(product: any): any {
  ...
  if (formatted.availablequantity !== undefined) {
    formatted.availablequantity = formatIntegerField(
...
export function formatPoinvoiceForAPI(poinvoice: any): any {
  ...
  logger.debug(
    {
      ...
      serializedPaymentDataConstructor:
        formatted.paymentdata?.constructor?.name,
    },
  ...
  if (formatted.transportationcharges !== undefined) {
    formatted.transportationcharges = formatNumericField(
      formatted.transportationcharges
    );
  }
  ...
  if (formatted.customdutytaxamount !== undefined) {
    formatted.customdutytaxamount = formatNumericField(
      formatted.customdutytaxamount
    );
  }
  ...
  if (formatted.paymentdata !== undefined) {
    ...
    if (formatted.paymentdata === null || formatted.paymentdata === undefined) {
      ...
      logger.debug(
        "formatPoinvoiceForAPI: Set paymentdata to null (was null/undefined)"
      );
    }
    // Handle string that needs parsing
      ...
      try {
        if (formatted.paymentdata.trim() === "") {
          formatted.paymentdata = null;
          logger.debug(
            "formatPoinvoiceForAPI: Set paymentdata to null (empty string)"
          );
        } else {
        ...
        if (originalPaymentData !== null && originalPaymentData !== undefined) {
          ...
          if (typeof originalPaymentData === "string") {
            try {
              ...
              logger.debug(
                "formatPoinvoiceForAPI: Parsed original string paymentdata"
              );
            } catch {
              formatted.paymentdata = null;
              logger.debug(
                "formatPoinvoiceForAPI: Failed to parse original string, set to null"
              );
            }
          } else if (Array.isArray(originalPaymentData)) {
            formatted.paymentdata = originalPaymentData;
            logger.debug(
              "formatPoinvoiceForAPI: Using original array paymentdata"
            );
            ...
            try {
              ...
              logger.debug(
                "formatPoinvoiceForAPI: Converted JSONB object via stringify/parse"
              );
            } catch {
              formatted.paymentdata = originalPaymentData;
              logger.debug(
                "formatPoinvoiceForAPI: Using original object as-is"
              );
            }
          } else {
            formatted.paymentdata = originalPaymentData;
          ...
          formatted.paymentdata = null;
          logger.debug(
            "formatPoinvoiceForAPI: Original was null/undefined, set to null"
          );
        }
      } else {
 ...
 */
export function formatSamplePurchaseRequestForAPI(
  samplePurchaseRequest: any
): any {
...
export function formatOrdersForAPI(order: any): any {
  ...
  if (formatted.orderprocessingtime !== undefined) {
    formatted.orderprocessingtime = formatIntegerField(
      formatted.orderprocessingtime
    );
  }
  ...
  if (formatted.readytodispatchdate !== undefined) {
    formatted.readytodispatchdate = formatIntegerField(
      formatted.readytodispatchdate
    );
  }
  ...
  if (formatted.paymentfaileddate !== undefined) {
    formatted.paymentfaileddate = formatIntegerField(
      formatted.paymentfaileddate
    );
  }

  // Handle productid array
  if (formatted.productid && Array.isArray(formatted.productid)) {
    formatted.productid = formatted.productid.map(
      (id: any) => formatIntegerField(id) || id
    );
  }
...
export function formatEntityForAPI(entity: any, entityType?: string): any {
  ...
  if (
    entity.invoiceamount !== undefined ||
  ...
  if (
    entity.orderlinenumber !== undefined ||
 ...
 */
export function formatEntitiesForAPI(
  entities: any[],
  entityType?: string
): any[] {
  ...
  product: [
    {
      ...
      displayTemplate: (record: any) =>
        `orderline ID ${record.id} (order ${record.orderid}, status: ${record.orderstatus})`,
    },
    {
      table: "stock",
      foreignKey: "puc",
      ...
      displayTemplate: (record: any) =>
        `stock ID ${record.id} (${record.stockstatus}${
          record.location || record.assetlocation
  ...
  supplier: [
    {
      ...
      displayTemplate: (record: any) =>
        `product ID ${record.id} (${record.name}, status: ${record.productstatus})`,
    },
    {
      table: "purchaseorder",
      foreignKey: "supplierid",
      ...
      displayTemplate: (record: any) =>
        `purchase order ${record.ponumber} (status: ${record.po_status})`,
    },
    {
      ...
      displayTemplate: (record: any) =>
        `purchase request ${record.prnumber} (status: ${record.prstatus})`,
    },
  ],
  stock: [
    {
  ...
  orders: [
    {
      ...
      displayTemplate: (record: any) =>
        `orderline ID ${record.id} (product: ${record.productname}, status: ${record.orderstatus})`,
    },
    {
      table: "stock",
      foreignKey: "orderid",
      ...
      displayTemplate: (record: any) =>
        `stock ID ${record.id} (${record.productname}, status: ${record.stockstatus})`,
    },
  ],
};
 ...
 */
async function identifyBlockingRecords(
  modelName: string,
  id: any
): Promise<{
  ...
  try {
    const relationships =
      MODEL_RELATIONSHIPS[modelName as keyof typeof MODEL_RELATIONSHIPS];
    ...
    try {
      ...
      logger.error(
        { error, modelName, id },
        "Error fetching main record for relationship check"
      );
      return {
        blockingRecords: [],
    ...
    if (blockingRecords.length === 0) {
      ...
      const summaryParts = Object.entries(tableGroups).map(
        ([table, records]) => {
    ...
    logger.error(
      { error: error.message, modelName, id },
      "Error identifying blocking records"
    );
    return {
      blockingRecords: [],
 ...
 */
export async function getConstraintViolationDetails(
  modelName: string,
  id: any,
  error: any
): Promise<{
  ...
  try {
    ...
    logger.error(
      { detailError: detailError.message, modelName, id },
      "Error getting constraint violation details"
    );
    return {
      specificMessage: `${modelName} ID ${id} cannot be deleted due to foreign key constraint`,
      blockingRecords: [],
...
export async function handleDeleteError(
  ...
  console.log(
    `=== ${modelName.toUpperCase()} DELETE ERROR STACK:`,
    error.stack
  );
  ...
  if (
    ...
    const constraintDetails = await getConstraintViolationDetails(
      modelName,
      id,
      error
    );
