...
export class OrderlineService {
  async findMany(
    ...
    try {
      logger.info(
        { filters, page, limit },
        "Starting dynamic orderline findMany with filters"
      );

      const { skip, take } = getPrismaSkipTake(page, limit);

      // Use the new dynamic filtering system
      const { data: orderlines, total } = await dynamicFindManyWithFilters(
        "orderline",
        filters,
        {
          skip,
      ...
      logger.info(
        {
          ...
          availableFields:
            orderlines.length > 0 ? Object.keys(orderlines[0]) : [],
        },
      ...
      logger.error(
        ...
        "Error in dynamic orderline findMany operation"
      );
      throw error;
    }
  }
  ...
  async findById(id: string) {
    try {
      logger.debug(
        { orderlineId: id },
        "Starting dynamic orderline findById operation"
      );
      ...
      logger.error(
        { error, orderlineId: id },
        "Error in orderline findById operation"
      );
      throw error;
    }
  }
  ...
  async findByOrderlineNumber(orderlinenumber: string) {
    try {
      logger.debug(
        { orderlinenumber },
        "Starting dynamic orderline findByOrderlineNumber operation"
      );
      ...
      logger.error(
        { error, orderlinenumber },
        "Error in orderline findByOrderlineNumber operation"
      );
      throw error;
    }
  }
  ...
  async findByOrderId(orderid: number) {
    try {
      logger.debug(
        { orderid },
        "Starting dynamic orderline findByOrderId operation"
      );
      ...
      logger.error(
        { error, orderid },
        "Error in orderline findByOrderId operation"
      );
      throw error;
    }
  }
  ...
  async create(data: CreateOrderlineInput & Record<string, any>) {
    try {
      logger.debug(
        { originalData: data },
        "Starting dynamic orderline create operation"
      );
      ...
      if (!orderline) {
        throw new Error(
          "Failed to create orderline - no valid fields provided"
        );
      }
  ...
  async update(id: string, data: UpdateOrderlineInput & Record<string, any>) {
    try {
      ...
      logger.debug(
        { originalData: data, orderlineId: id },
        "Starting dynamic orderline update operation"
      );
      ...
      const orderline = await dynamicUpdate(
        "orderline",
        { id: parseInt(id) },
        updateData
      );
      ...
      if (!orderline) {
        throw new Error(
          "Failed to update orderline - no valid fields provided"
        );
      }
      ...
      logger.error(
        { error, data, orderlineId: id },
        "Error in orderline update operation"
      );
      throw error;
    }
  }
  ...
  async delete(id: string) {
    try {
      ...
      logger.debug(
        { orderlineId: id },
        "Starting dynamic orderline delete operation"
      );
      ...
      logger.info(
        { orderlineId: id },
        "Dynamic orderline delete completed successfully"
      );
    } catch (error) {
      logger.error(
        { error, orderlineId: id },
        "Error in orderline delete operation"
      );
      throw error;
    }
  }
  ...
  async upsert(data: UpsertOrderlineInput & Record<string, any>) {
    try {
      ...
      if (id) {
        ...
        logger.debug(
          { orderlineId: id, data: updateData },
          "Upserting existing orderline"
        );
  ...
  async updateOrderlineStatus(
    ...
    try {
      logger.debug(
        { orderlineId: id, status, additionalData },
        "Starting orderline status update operation"
      );
      ...
      logger.error(
        { error, orderlineId: id, status },
        "Error in orderline status update operation"
      );
      throw error;
    }
  }
  ...
  async bulkUpdateStatus(
    ...
    try {
      logger.debug(
        { orderlineIds, status, additionalData },
        "Starting bulk orderline status update operation"
      );

      const results = [];
      for (const id of orderlineIds) {
        try {
          const result = await this.updateOrderlineStatus(
            id,
            status,
            additionalData
          );
          ...
          logger.error(
            { error, orderlineId: id },
            "Error updating individual orderline status"
          );
      ...
      logger.error(
        { error, orderlineIds, status },
        "Error in bulk orderline status update operation"
      );
      throw error;
    }
  }

  async cancelOrderline(id: string, reason?: string) {
    try {
      logger.info(
        { orderlineId: id, reason },
        "Starting orderline cancellation process"
      );
      ...
      const productUpdates = await this.restoreProductQuantities(
        productId,
        originalQuantity
      );
      ...
      logger.error(
        { error, orderlineId: id },
        "Error in orderline cancellation process"
      );
      throw error;
    }
  }

  private async restoreProductQuantities(productId: number, quantity: number) {
    try {
      logger.debug(
        { productId, quantity },
        "Starting product quantity restoration"
      );
      ...
      if (!product) {
        logger.warn(
          { productId },
          "Product not found for quantity restoration"
        );
        return [
          {
            productId,
  ...
  private async checkAndUpdateOrderStatus(orderId: number) {
    try {
      logger.debug(
        { orderId },
        "Checking if all orderlines are cancelled to update order status"
      );
      ...
      const allCancelled = orderlines.every(
        (orderline) => orderline.orderstatus === "cancelled"
      );
      ...
      logger.debug(
        {
          ...
          cancelledOrderlines: orderlines.filter(
            (ol) => ol.orderstatus === "cancelled"
          ).length,
          allCancelled,
        },
      ...
      if (allCancelled) {
        ...
        const updatedOrder = await dynamicUpdate(
          "orders",
          { id: orderId },
          orderUpdateData
        );
        ...
        logger.debug(
          { orderId },
          "Not all orderlines are cancelled, order status remains unchanged"
        );
        return false;
      }
    } catch (error) {
      logger.error(
        { error, orderId },
        "Error checking and updating order status"
      );
      return false;
    }
  }
