...
export class UsersService {
  ...
  async findByMobileNumber(mobileNumber: number) {
    try {
   ...
   */
  async deactivateAccount(userId: number, email?: string): Promise<any> {
    try {
   ...
   */
  async authenticateByMobile(mobileNumber: number, password: string): Promise<{ user: any; token: string } | null> {
    try {
   ...
   */
  async generateMobileOTP(mobileNumber: number, verifyOnly: boolean = false): Promise<{ otp: number; expiresAt: Date; isNewUser?: boolean } | null> {
    try {
  ...
  private clearOTP(mobileNumber: number): void {
    this.otpStorage.delete(mobileNumber);
  }
   ...
   */
  async createGuestUser(data: CreateGuestUserInput & Record<string, any>) {
    try {
   ...
   */
  async convertGuestToRegistered(userId: number, registrationData?: Partial<CreateUsersInput>) {
    try {
   ...
   */
  async findOrdersByMobileNumber(mobileNumber: number) {
    try {
   ...
   */
  async mergeGuestUser(guestUserId: number, authenticatedUserId: number) {
    try {
  ...
  async storeAmazonRefreshToken(
    ...
    refreshToken: string,
    sellerId: string,
    userType: string = 'inventoryusers',
    marketplaceId: string = 'A21TJRUUN4KGV'
  ) {
    try {
      ...
      const existing = await (prisma as any).amazonConnection.findFirst({
        where: {
          userId,
          sellerId,
          userType,
      ...
      if (existing) {
        // Update existing
        connection = await (prisma as any).amazonConnection.update({
        ...
        // Create new
        connection = await (prisma as any).amazonConnection.create({
          data: {
            userId,
            userType,
            sellerId,
   ...
   */
  async getAmazonRefreshToken(
    userId: number,
    userType: string = 'inventoryusers'
  ): Promise<string | null> {
    try {
      const connection = await (prisma as any).amazonConnection.findFirst({
        where: {
          userId,
          userType,
        },
   ...
   */
  async deleteAmazonConnection(
    userId: number,
    userType: string = 'inventoryusers'
  ): Promise<boolean> {
    try {
      const result = await (prisma as any).amazonConnection.deleteMany({
        where: {
          userId,
          userType,
        },
