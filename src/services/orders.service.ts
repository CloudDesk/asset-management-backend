...
export class OrdersService {
  async findMany(
    ...
    try {
      logger.info(
        { filters, page, limit },
        "Starting dynamic orders findMany with filters"
      );

      const { skip, take } = getPrismaSkipTake(page, limit);

      // Use the new dynamic filtering system
      const { data: orders, total } = await dynamicFindManyWithFilters(
        "orders",
        filters,
        {
          skip,
      ...
      logger.error(
        ...
        "Error in dynamic orders findMany operation"
      );
      throw error;
    }
  }
  ...
  async findById(id: Number) {
    try {
      logger.debug(
        { orderId: id },
        "Starting dynamic orders findById operation"
      );
      ...
      logger.error(
        { error, orderId: id },
        "Error in orders findById operation"
      );
      throw error;
    }
  }
  ...
  async findByOrderId(orderid: string) {
    try {
      logger.debug(
        { orderid },
        "Starting dynamic orders findByOrderId operation"
      );
      ...
      logger.error(
        { error, orderid },
        "Error in orders findByOrderId operation"
      );
      throw error;
    }
  }
  ...
  async create(data: CreateOrdersInput & Record<string, any>) {
    try {
      logger.debug(
        { originalData: data },
        "Starting dynamic orders create operation"
      );
  ...
  async createFromCartItems(cartItems: any[]) {
    try {
      logger.debug(
        { cartItems, itemCount: cartItems.length },
        "Starting order creation from cart items"
      );
      ...
      logger.error(
        { error, cartItems },
        "Error in order creation from cart items"
      );
      throw error;
    }
  }
  ...
  async createOrderlinesFromOrderItems(
    ...
    for (let i = 0; i < orderItems.length; i++) {
      ...
      const orderlineData = {
        ...
        productamount:
          parseFloat(orderItem.productamount?.toString() || "0") || null,
        discountamount:
          parseFloat(orderItem.discountamount?.toString() || "0") || null,
        orderamount:
          parseFloat(orderItem.orderamount?.toString() || "0") || null,
  ...
  async update(id: string, data: UpdateOrdersInput & Record<string, any>) {
    try {
      ...
      logger.debug(
        { originalData: data, orderId: id },
        "Starting dynamic orders update operation"
      );
      ...
      const order = await dynamicUpdate(
        "orders",
        { id: parseInt(id) },
        updateData
      );
      ...
      logger.error(
        { error, data, orderId: id },
        "Error in orders update operation"
      );
      throw error;
    }
  }
  ...
  async delete(id: string) {
    try {
      ...
      logger.info(
        { orderId: id },
        "Dynamic orders delete completed successfully"
      );
    } catch (error) {
  ...
  async upsert(data: UpsertOrdersInput & Record<string, any>) {
    try {
      ...
      if (id) {
        ...
        logger.debug(
          { orderId: id, data: updateData },
          "Upserting existing order"
        );
  ...
  async updateOrderStatus(
    ...
    try {
      logger.debug(
        { orderId: id, status, additionalData },
        "Starting orders status update operation"
      );
      ...
      logger.error(
        { error, orderId: id, status },
        "Error in orders status update operation"
      );
      throw error;
    }
  }
