...
export class PhonePeService {
   ...
   */
  private initializeSDKClient(): void {
    try {
      if (
   ...
   */
  async initiatePayment(paymentRequest: PhonePePaymentRequest): Promise<{
    success: boolean;
    message: string;
    redirectUrl?: string;
    transactionId?: string;
    ...
    try {
      ...
      if (this.sdkClient) {
        return await this.initiatePaymentWithSDK(
          paymentRequest,
          finalCallbackUrl
        );
      } else {
        return await this.initiatePaymentLegacy(
          paymentRequest,
          finalCallbackUrl
        );
      }
   ...
   */
  private async initiatePaymentWithSDK(
   ...
   */
  private async initiatePaymentLegacy(
    paymentRequest: PhonePePaymentRequest,
    callbackUrl: string
  ): Promise<{
    success: boolean;
    ...
    const { merchantTransactionId, amount, name, mobileNumber } =
      paymentRequest;
   ...
   */
  private async checkPaymentStatusWithSDK(
    merchantTransactionId: string
  ): Promise<PaymentStatusResponse> {
    try {
      logger.info(
        { merchantTransactionId },
        "Using SDK to check payment status"
      );

      const orderStatus = await this.sdkClient!.getOrderStatus(
        merchantTransactionId
      );
      ...
      logger.info(
        { merchantTransactionId },
        "Falling back to legacy status check"
      );
      return await this.checkPaymentStatusLegacy(merchantTransactionId);
    }
  }
   ...
   */
  private async checkPaymentStatusLegacy(
    merchantTransactionId: string
  ): Promise<PaymentStatusResponse> {
    try {
      logger.info(
        { merchantTransactionId },
        "Using legacy method to check payment status"
      );
  ...
  async createSdkOrder(orderData: {
    ...
    try {
      ...
      try {
        ...
        console.log(
          "Last 20 chars:",
          jwtToken?.substring(jwtToken.length - 20)
        );
        console.log("============================");
  ...
  async refundPayment(
    ...
    try {
      ...
      if (this.sdkClient) {
        return await this.refundPaymentWithSDK(
          merchantTransactionId,
          refundId,
          finalRefundAmount,
          reason,
   ...
   */
  private async refundPaymentWithSDK(
    merchantTransactionId: string,
    refundId: string,
    refundAmount: number,
   ...
   */
  private async refundPaymentLegacy(
    merchantTransactionId: string,
    refundId: string,
    refundAmount: number,
    reason: string | undefined,
  ...
  private async createInitialTransaction(transactionData: any) {
    try {
