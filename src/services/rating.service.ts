...
export module ratingService {
  ...
  export const deleteImage = async (request: any, reply: any) => {
    try {
      let querydata: string = '';
      let params: any[];
      let ratingData = request.body;
  ...
  export const deleteRating = async (id: number) => {
    try {
      const result: any = await query(`DELETE FROM rating where id = $1`, [id])
  ...
  export const updateAvgRating = async (productid: number) => {
    try {
