...
export const completeReturnResolutionSchema = z.object({
  ...
  refund_destination: z.enum(['original_sources', 'wallet']).optional(),
  consent_accepted: z.coerce.boolean().optional().default(false),
  consent_channel: z.enum(['call', 'whatsapp', 'email', 'support_ticket', 'in_app', 'other']).optional(),
  consent_reference: z.string().trim().max(500).optional(),
  consent_notes: z.string().trim().max(3000).optional(),
